...
public class TopAdminServiceImpl extends ServiceImpl<TopAdminMapper, TopAdmin> implements TopAdminService {
    private final TopAdminMapper topAdminMapper;

    public TopAdminServiceImpl(TopAdminMapper topAdminMapper) {
        this.topAdminMapper = topAdminMapper;
    }
//    @Override
...
//    }
    ...
    public TopAdmin adminLogin(TopAdminDto topAdminDto) throws AccountNotFoundException {
        ...
        TopAdmin topAdmin = topAdminMapper.getByAdminName(adminName);

        if(topAdmin ==  null){
            //账号不存在
            throw new AccountNotFoundException(ExceptionConstant.ACCOUNT_NOT_FOUND);
        }
        if (!pswd.equals(topAdmin.getPassword())){
